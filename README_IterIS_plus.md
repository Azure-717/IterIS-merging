...
将各向同性正则化 (αI) 替换为基于激活协方差的曲率感知正则化（遵循岭回归原理）：
- 在激活方差**低**的方向（矩阵接近奇异）施加**较高**正则化以保证数值稳定
- 在激活方差**高**的方向（信噪比高）施加**较低**正则化以保留主要信号
- 支持 DCS 权重协同：高冲突样本不参与协方差计算（加权最小二乘）

**理论依据（Ridge Regression 原理）：**
- 这是闭式求解 (Closed-form Solving)，不是 SGD 微调
...
| 正则化策略 | 各向同性 | 曲率感知 | CAMR 基于岭回归原理 |
| 超参数敏感度 | σ 固定 | σ 自适应 | DCS 自动调整 |
| 模块协同 | 独立运行 | 权重传递 | DCS→CAMR 协同 (WLS) |
...
CAMR 遵循岭回归 (Ridge Regression) 原理用于闭式求解：
- 目标：解决矩阵求逆时的数值稳定性问题
- 高方差方向：数据充足，信噪比高，矩阵可逆 → 少正则化
- 低方差方向：数据不足，接近奇异 → 多正则化"撑起"该维度
- 正则化矩阵：Λ = α(1 - diag_norm) + β
